...
class HomeViewController: UIViewController {
    ...
    fileprivate let greenBackOn = UIColor(red:0.17,
                                          green:0.67,
                                          blue:0.48,
                                          alpha:1.00)
    fileprivate let greenBackOff = UIColor(red:0.17,
                                           green:0.55,
                                           blue:0.40,
                                           alpha:1.00)
...
class HomeViewModel {
    private let matcher = AppDelegate.RunAcrConfig.matcher //TODO: Inject

    var isButtonOn: Bool {
        get { return self.matcher.enabled }
        set { self.matcher.enabled = newValue }
    }
